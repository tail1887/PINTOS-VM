...
- 주의: `arg_addrs[ARG_MAX]`는 커널 스택 지역 배열 대신 page 할당으로 보관한다.
...
- 규칙 3: 문자열 시작 주소를 저장하는 `arg_addrs`는 `palloc_get_page()`로 할당하고 함수 종료 전 해제한다.
- 금지 1: `char *arg_addrs[ARG_MAX]`를 지역 변수로 두지 않는다. PintOS 커널 스택은 thread page 안에 있어 큰 배열이 `struct thread` magic을 덮을 수 있다.

구현 체크 순서:

1. `arg_addrs` page를 할당한다.
2. 문자열을 역순으로 push하고 시작 주소를 `arg_addrs[]`에 기록한다.
3. 8바이트 정렬을 맞춘 뒤 `NULL` 센티널을 push한다.
4. `arg_addrs[]`를 역순 순회해 `argv` 포인터들을 push한다.
5. 포인터 배열 시작 주소를 `argv_user_addr`에 저장한다.
6. 마지막에 fake return address(0)를 push하고 `user_if->rsp`를 갱신한다.
7. 성공/실패 경로에서 `arg_addrs` page를 해제한다.
...
- 커널 스택 회귀: `open-empty`, `write-normal`, `wait-simple`
- 진단: `hex_dump()`로 문자열/포인터/정렬 구간을 한 번에 확인

`wait()` 구현 이후 기존 argument passing 테스트가 아닌 file/exec/wait 테스트에서 갑자기 `is_thread(t)` panic이 발생하면, wait child list보다 먼저 `process_exec()`와 `build_user_stack_args()`의 큰 지역 배열 여부를 확인합니다.