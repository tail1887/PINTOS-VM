...
### 무엇을 연결하는가 (기술 맥락)
`pintos/userprog/process.c`의 `load_segment()`, `lazy_load_segment()`, ELF의 file offset/read_bytes/zero_bytes/writable 정보, `pintos/vm/vm.c`의 `vm_alloc_page_with_initializer()`, `threads/vaddr.h`의 `PGSIZE`를 연결합니다.
### 완성의 의미 (결과 관점)
...
1. load 시점에는 SPT 등록만 한다.
2. fault 시 SPT에서 page를 찾아 claim한다.
3. `lazy_load_segment`에서 read와 zero fill을 수행한다.
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/userprog/process.c`의 `load_segment()`
- 위치: `pintos/threads/vaddr.h`의 `PGSIZE`
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/userprog/process.c`의 `lazy_load_segment()`
...
- 규칙 2: page_zero_bytes는 해당 page에서 파일로 채우지 않는 바이트 수와 일치시킨다.
...
- 규칙 1: 파일 읽기 정책은 한 가지 방식으로 일관되게 유지되어야 한다.
