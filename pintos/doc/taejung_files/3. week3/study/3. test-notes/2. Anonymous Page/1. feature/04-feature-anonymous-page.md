...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `vm/anon.c`의 `anon_initializer()`
- 위치: `include/vm/anon.h`의 `struct anon_page`
...
- 위치: `vm/vm.c`의 claim 경로

## 5. 구현 주석

### 5.1 `anon_initializer()`

#### 5.1.1 `anon_initializer()`에서 anonymous page metadata 초기화
- 수정 위치: `vm/anon.c`의 `anon_initializer()`
...
#### 5.2.1 `anon_swap_out()`에서 anonymous page를 swap slot에 저장
...
#### 5.3.1 `anon_swap_in()`에서 swap slot 내용을 frame으로 복구
