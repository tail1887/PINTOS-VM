...
### 무엇을 연결하는가 (기술 맥락)
`pintos/userprog/exception.c`의 `page_fault()`, `struct intr_frame.rsp`, `pintos/vm/vm.c`의 `vm_try_handle_fault()`, `vm_stack_growth()`, `spt_find_page()`를 연결합니다.
### 완성의 의미 (결과 관점)
...
- 선택: rsp 근접 조건과 stack limit을 함께 적용한다(`03-feature-stack-limit-and-invalid-access.md`).
...
1. SPT hit면 stack growth 분기 전에 기존 page claim이다.
2. SPT miss일 때만 rsp 근접을 본다.
3. 근접하면 stack 하한 검사 후 growth로 넘긴다.
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/vm.c`의 `vm_try_handle_fault()`
- 위치: `pintos/userprog/exception.c`의 fault context 판별
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/vm.c`의 `vm_try_handle_fault()`
- 위치: `pintos/vm/vm.c`의 `spt_find_page()`, `vm_stack_growth()`
...
- 규칙 2: fault address가 rsp 근처여야 한다(팀이 정한 여유 포함).
- 규칙 3: 최대 stack 크기(하한)를 넘지 않아야 한다.
