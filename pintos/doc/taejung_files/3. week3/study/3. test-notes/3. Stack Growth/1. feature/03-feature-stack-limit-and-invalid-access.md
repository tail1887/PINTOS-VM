...
### 무엇을 연결하는가 (기술 맥락)
`USER_STACK`, stack maximum size, `is_user_vaddr()`, `pintos/vm/vm.c`의 `vm_try_handle_fault()`, `vm_stack_growth()`, SPT miss 정책을 연결합니다.
### 완성의 의미 (결과 관점)
...
1. user·하한·rsp 근접을 순서대로 검사한다.
2. 통과하면 `vm_stack_growth`로 page를 넣는다.
3. 실패하면 VM handler가 false로 끝나 프로세스가 죽는 경로로 간다.
...
#### 개념 설명
정상 stack growth로 판정되면 fault address가 속한 page를 anonymous writable page로 등록해야 합니다. page 단위로 만들지 않으면 같은 stack page 내부 접근에서 SPT lookup이 흔들립니다.
#### 시퀀스 및 흐름
```mermaid
flowchart LR
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/vm.c`의 `vm_stack_growth()`
- 위치: `pintos/vm/vm.c`의 `vm_alloc_page()`, `vm_claim_page()`
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/vm.c`의 `vm_try_handle_fault()`
- 위치: `pintos/userprog/exception.c`의 fault 실패 처리
...
2. `vm_alloc_page(VM_ANON | VM_MARKER_0, upage, true)` 또는 팀 구현의 anonymous page 생성 규약을 호출한다.
