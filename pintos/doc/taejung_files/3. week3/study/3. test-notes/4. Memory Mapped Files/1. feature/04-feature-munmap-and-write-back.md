...
### 무엇을 연결하는가 (기술 맥락)
`pintos/userprog/syscall.c`의 `sys_munmap()`, mapping metadata, pml4 dirty bit, `pintos/vm/file.c`의 `do_munmap()`, `file_backed_swap_out()`, `file_backed_destroy()`, `pintos/vm/vm.c`의 `supplemental_page_table_kill()`을 연결합니다.
### 완성의 의미 (결과 관점)
...
1. 시작 addr로 mapping 전체 범위를 찾는다.
2. page별로 dirty면 write-back한다.
3. pml4·SPT·metadata를 한 번만 정리한다.
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/userprog/syscall.c`의 `sys_munmap()`
- 위치: `pintos/vm/file.c`의 `do_munmap()`
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/file.c`의 `file_backed_swap_out()`
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/file.c`의 `do_munmap()`, `file_backed_destroy()`
...
- 위치: `pintos/vm/file.c`의 `file_backed_destroy()`
