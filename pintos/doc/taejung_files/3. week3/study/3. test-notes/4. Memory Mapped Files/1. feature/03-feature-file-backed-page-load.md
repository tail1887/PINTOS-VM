...
### 무엇을 연결하는가 (기술 맥락)
`pintos/vm/file.c`의 `file_backed_initializer()`, `file_backed_swap_in()`, file offset, read_bytes, zero_bytes metadata를 연결합니다.
### 완성의 의미 (결과 관점)
...
1. initializer로 file-backed operation을 연다.
2. `swap_in`에서 read와 zero fill을 수행한다.
3. 파일 끝 너머는 0으로 유지한다.

## 4. 기능별 가이드 (개념/흐름 + 구현 주석 위치)
### 4.1 기능 A: file-backed page operation 설정
#### 개념 설명
file-backed page는 backing file이 있습니다. fault나 eviction에서 file 전용 swap-in/out 정책을 쓰도록 page operation table을 file-backed 전용으로 설정해야 합니다.
#### 시퀀스 및 흐름
```mermaid
flowchart LR
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/file.c`의 `file_backed_initializer()`
- 위치: `pintos/include/vm/file.h`의 `struct file_page`
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/file.c`의 `file_backed_swap_in()`
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/vm/file.c`의 `file_backed_swap_in()`
- 위치: mmap aux 또는 `struct file_page` metadata 정의
