...
### 무엇을 연결하는가 (기술 맥락)
`pintos/userprog/syscall.c`의 `sys_mmap()`, fd table, `file_reopen()`, `pintos/vm/vm.c`의 SPT insert, `pintos/vm/file.c`의 `do_mmap()`과 file-backed page initializer를 연결합니다.
### 완성의 의미 (결과 관점)
유효한 mmap은 시작 주소를 반환하고, 실패 조건은 `NULL`/명세된 실패값으로 처리됩니다.
...
1. 검증 후에만 page 등록을 시작한다.
2. overlap이 있으면 전체 실패한다.
3. page마다 lazy VM_FILE 등록한다.
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `pintos/userprog/syscall.c`의 `sys_mmap()`
