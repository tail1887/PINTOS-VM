...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `vm/vm.c`의 `vm_get_victim()`
- 위치: `threads/mmu.c` 또는 `threads/pml4.h`의 accessed bit helper
...
#### 구현 주석 (보면 되는 함수/구조체)
- 위치: `vm/vm.c`의 `vm_evict_frame()`
- 위치: `vm/anon.c`의 `anon_swap_out()`, `vm/file.c`의 `file_backed_swap_out()`
...
#### 5.1.1 `vm_get_victim()`에서 clock 알고리즘으로 victim 선정
- 수정 위치: `vm/vm.c`의 `vm_get_victim()`
...
#### 5.2.1 `vm_evict_frame()`에서 victim page를 backing store로 내보내기
- 수정 위치: `vm/vm.c`의 `vm_evict_frame()`
...
2. `page->operations->swap_out(page)`를 호출해 anonymous/file-backed 정책에 따라 내용을 보존한다.
