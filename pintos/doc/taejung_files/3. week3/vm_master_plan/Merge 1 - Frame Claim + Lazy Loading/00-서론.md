...
| 4.3 | 함수별 구현 주석 (고정안) | 역할 문단 + **흐름** 번호 목록 + (선택) 플로우차트 |
| 4.4 | 함수 간 연결 순서 | 호출 체인 고정 |
| 4.5 | 실패 처리/롤백 규칙 | 예외/실패 시 행동 고정 |
| 4.6 | 완료 체크리스트 | 구현 완료 판정 고정 |

#### §4.3.0 분업 문서 안에서의 적는 방식 (이 Merge 기준)

- 함수(또는 논리 단위)마다 **짧은 역할 문단**(코드 펜스로 감싼 “추상 전용” 블록은 쓰지 않는다)과 **`흐름` 아래 `1. 2. 3. …` 번호 목록** 한 블록을 둔다. 앞쪽 번호는 맥락·순서, 뒤쪽 번호는 API·분기·실패·해제처럼 리뷰할 디테일에 가깝게 둔다.
- **플로우차트 (B안, 선택)**: **루프·다중 분기·실패/롤백 경로**가 길어서 번호만으로 헷갈릴 때만 Mermaid `flowchart TD`를 추가한다. 선형 몇 단계 수준은 번호만으로 충분하다. 노드 라벨은 **한 줄**로 쓰고 HTML 줄바꿈 태그는 쓰지 않는다. 참고: `C - Executable Segment Lazy Loading.md` §4.3.
- **하지 않음** 규칙은 번호 목록에 넣기 어렵면, **그림·흐름 밖에 한 줄**로 “C 경계”처럼 고정해도 된다.
- §4.5·§4.6에서 다른 함수를 가리킬 때는 **「§4.3 `함수명` 흐름 n」** 형태로 통일한다.
